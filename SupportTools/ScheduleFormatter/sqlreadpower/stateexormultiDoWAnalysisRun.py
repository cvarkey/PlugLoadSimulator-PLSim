...
def savingsreporting(inputrange, analysisvalues, averagebase, standbycomputerwatt, activecomputerwatt, standbyaccessorieswatt, activeaccessorieswatt, accessorycontrol, simPMsavingsval, simPMsavingsOn, energyreport):
    ...
    for x in analysisvalues:
        ...
        if (energyreport == True):
            ...
            sch3 = ((((deltawatt+deltawattaccessories)/1000)*runtimehr)*261)+(104*runtimehr*(deltawatt/1000))
...
for q, valuesubject in enumerate(subjectlist):
    ...
    for rowindex, row in enumerate(queryreturn): #go thru query and generate a full list of states from the dataset
        ...
        if (dayanalysis==0): 
            if (('Sunday'in row[day_of_weekrow]) or ('Monday'in row[day_of_weekrow]) or ('Tuesday'in row[day_of_weekrow]) or ('Wednesday'in row[day_of_weekrow]) or ('Thursday'in row[day_of_weekrow]) or ('Friday'in row[day_of_weekrow]) or ('Saturday'in row[day_of_weekrow]) or ('Sunday'in row[day_of_weekrow])):        
                datetallylist.append(row[daterow]) 
        elif (dayanalysis==1):
            if (('Monday'in row[day_of_weekrow]) or ('Tuesday'in row[day_of_weekrow]) or ('Wednesday'in row[day_of_weekrow]) or ('Thursday'in row[day_of_weekrow]) or ('Friday'in row[day_of_weekrow]) or ('Saturday'in row[day_of_weekrow])):        
                datetallylist.append(row[daterow])
            
        elif (dayanalysis==2):     
            if (('Sunday'in row[day_of_weekrow]) or ('Saturday'in row[day_of_weekrow])):
                datetallylist.append(row[daterow])
    ...
    for listeddate in datetallylist_unique_list: # display entries for a single date
        ...
        for rowindex, row in enumerate(queryreturn): #page thru all returned rows from query, also return an index number related to the row
            ...
            if ((('On') in row[stateposition]) and (row[daterow] == listeddate)):
                ...
                if ((('On') in row[stateposition]) and (row[daterow] == listeddate)):    
                    #print ("Records: ")
                    #print (len(ontimelist))  #print number of actions, helpful for debug
                    print() #Newline between rows - makes it formatted properly when there is final readout   


    
        
        if ((len(ontimelist) == len(activetimelist)) and (len(activetimelist) !=0 or len(ontimelist) !=0)): #check to see if there is a list for the same day for both active and ON states #check to see if there is a list for the same day for both active and ON states
    ...
    perdayidleaverage = idleaverage/(resultsreviewcount+.0000001)
